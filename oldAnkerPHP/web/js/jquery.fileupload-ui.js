...
(function ($) {
    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        ...
        _deleteHandler: function (e) {
            e.preventDefault();
            var button = $(this);
            var dokument = button.closest('.template-download');
            //if(e.hasOwnProperty('originalEvent'))
            //   -> real click
            //   
            // Löschen ohne Nachfrage, wenn:
            // - fehlerhafter Upload 
            // - oder aus dem Quelltext mit .click() aufgerufen (alle Löschen)
            if(button.closest('.template-download').hasClass('ui-state-error') 
                || !e.hasOwnProperty('originalEvent')
                ||
                (   e.hasOwnProperty('originalEvent') && 
                    confirm("Wollen Sie die Datei wirklich löschen?")   
                )
                ) 
            {         
                var i = dokument.attr('id').split('_');
                i = i[i.length - 1];
                ...
                e.data.fileupload._trigger('destroy', e, {
                    ...
                    data: {
                        id: i, 
                        _csrf_token: dokument.attr('token')
                    },
                    fail: function() {
        ...
        _editHandler: function (e) {
            ...
            var titleField = $(this).closest('.template-download').find('.title');     
            var titleText = titleField.text();
            titleField.text('');
            titleField.append('<label>Titel (Pflichtfeld):</label><br/><input name="title[]" type="text" value="' + titleText + '" required>');
            // hide edit button and display the others
            $(this).hide();
            $(this).closest('.template-download').find('td.delete button').hide();
            $(this).closest('.template-download').find('td.save button').show();
            $(this).closest('.template-download').find('td.abort button').show();
        },
        
        _abortHandler: function (e) {
            e.preventDefault();            
            
            var id = $(this).closest('.template-download').get(0).id.split('_');
            id = id[id.length - 1];    
            $('#ajax-loader_' + id).show();         
            var title = $(this).closest('.template-download').find('.title');
            
            $.getJSON(app + 'dokumente/gettitle/' + id, function (data) {
                title.text(data);
                $('#ajax-loader_' + id).hide();  
            });
            
            // hide abort and save button and display the others
            $(this).hide();
            $(this).closest('.template-download').find('td.save button').hide();
            $(this).closest('.template-download').find('td.edit button').show();
            $(this).closest('.template-download').find('td.delete button').show();
        },
        
        _saveHandler: function (e) {
            e.preventDefault();
            var dokument = $(this).closest('.template-download');
            var id = dokument.get(0).id.split('_');
            id = id[id.length - 1];            
            var title = dokument.find('.title');
            var text = title.find('input').val();
            ...
            if(text) {
                ...
                // hide abort and save button and display the others
                $(this).hide();
                $(this).closest('.template-download').find('td.abort button').hide();
                $(this).closest('.template-download').find('td.edit button').show();
                $(this).closest('.template-download').find('td.delete button').show();
            }
        },
        ...
        _initFileUploadButtonBar: function () {
            ...
            this.element.find('.fileinput-button').each(function () {
                ...
                .bind('click.' + ns, function (e) {
                    e.preventDefault();
                    if (confirm("Wollen Sie wirklich alle Dateien löschen?")) {
